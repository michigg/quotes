...
function cloneMore(selector, prefix) {
    ...
    var conditionRow = $('.conversation-particle:not(:last)');
...
function deleteForm(prefix, btn) {
    ...
    if (total > 1){
        btn.closest('.conversation-particle').remove();
        var forms = $('.conversation-particle');
...
$(document).on('click', '.add-form-row', function(e){
    e.preventDefault();
    cloneMore('.conversation-particle:last', 'form');
    return false;
});
